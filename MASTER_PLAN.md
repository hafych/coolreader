...
- [ ] ~~Добавить безопасную еженедельную проверку возможности синхронизации с
  `buggins/coolreader` без push и изменения веток.~~
...
- [ ] Сделать именованные пошаговые миграции БД с обязательными postconditions;
  не подавлять ошибки обязательных DDL/DML через `execSQLIgnoreErrors()`.
- [ ] Добавить fixture-базу для каждой поддерживаемой старой версии и проверить:
  upgrade, rollback при ошибке, повторный запуск и отказ от future schema.
- [ ] Сделать backup/restore атомарным: временный файл, `fsync`, rename, проверка
  целостности и политика хранения нескольких поколений.
- [ ] Описать жизненный цикл старых полей OPDS credentials и физически удалить их
  из схемы в контролируемой миграции.
...
- [ ] Ограничить ZIP на уровне всего архива: число entries, суммарный распакованный
  размер, вложенность, нормализация путей и дубликаты имён.
...
- [ ] После первого успешного dry-run разобрать конфликтные зоны и определить
  периодичность реальной синхронизации с upstream.

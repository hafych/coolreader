...
- [-] Зафиксировать политику владения нативными объектами и постепенно заменить
  ...
  generic value-array, reference-vector and owning/borrowed pointer-vector
  storage, reference-cache buckets/index exports and bounded map slots,
  longjmp-safe PNG rows/pixels, JPEG pool/error lifecycle, а также draw mark
  list. Остальные участки мигрируются отдельными bounded-пакетами.

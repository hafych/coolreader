...
| Android API 21 minimum, pinned NDK, guarded release signing, least-privilege manifest, obsolete billing removal, blocking lint and emulator CI | fork-only for now | Establishes the downstream support/privacy baseline, keeps signing material external, avoids claiming an unsupported purchase flow and verifies startup, file, SAF, TTS-action and reinstall behavior | Reclassify when upstream adopts the same support matrix, signing boundary, monetization baseline, permission policy and instrumentation coverage |

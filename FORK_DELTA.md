...
| Android API 21 minimum, pinned NDK, least-privilege manifest, blocking lint and emulator CI | fork-only for now | Establishes the downstream support/privacy baseline and startup, file, SAF, TTS-action and reinstall instrumentation | Reclassify when upstream adopts the same support matrix, permission policy and instrumentation coverage |
